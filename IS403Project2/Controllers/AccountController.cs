            ...
            switch (result)
            {
                case SignInStatus.Success:
                    if(loginInfo.Login.LoginProvider == "Google")
                    {
                        ...
                        Response.Cookies["firstName"].Expires = DateTime.Now.AddHours(1);
                        Response.Cookies["lastName"].Value = lastname;
                        Response.Cookies["lastName"].Expires = DateTime.Now.AddHours(1);

                        var oAccount = db.Database.SqlQuery<Users>("SELECT * FROM Users WHERE userEmail = '" + email + "'").SingleOrDefault();
                        if(oAccount == null)
                        {
                            db.Database.ExecuteSqlCommand("INSERT INTO Users (userEmail,userPassword,userFirstName,userLastName) VALUES ('" + email + "','password','"+ firstName +"','"+ lastname +"')");
                        }
                    }
                    return RedirectToLocal(returnUrl);
                case SignInStatus.LockedOut:
